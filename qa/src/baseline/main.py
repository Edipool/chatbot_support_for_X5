...
class Preprocesser:
    ...
    def preprocess(self, text, custom_bigrams=None):
        ...
        text = list(filter(lambda tok: not str.isspace(tok), self.lemmatizer.lemmatize(' '.join(text))))
...
def get_custom_bigrams(train_df, 
                       ranking:Literal['freq', 'tscore', 'pmi', 'llr']='freq'):
    texts_question = []
...
class ChatbotModel:
    ...
    def __init__(self, data, label_encoder: LabelEncoder, bigram_ranking='freq'):
        ...
        texts = [' '.join(self.preprocess(text)) 
                 for text in texts]
    ...
    def evaluate(self, test_questions, target_answers, method='tfidf'):
        ...
        precision = precision_score(target_answers_encoded, predicted_answers_encoded, average='macro', zero_division=1)
...
if __name__ == '__main__':
    ...
    train_df, test_df = train_test_split(df, test_size=0.15, random_state=42, stratify=df['content'])
